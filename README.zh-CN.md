...
[English Guide](README.en.md) | [项目主页](README.md)
...
应用会从启动日志中识别 `localhost`、`127.0.0.1`、`0.0.0.0` 或局域网 IP 的 HTTP 地址；同一条启动命令若输出多个本地地址（例如前后端端口），会全部展示并可分别打开。若界面一直显示“等待启动日志输出 Local 地址”，请先检查日志中是否实际输出了带端口的完整 URL。
...
桌面应用启动后只请求 GitHub Release 中的 `latest.json` 检查版本，不会自动下载安装包。工作台顶部提供“检查更新”按钮；发现新版本后按钮会切换为对应版本的更新入口，只有用户再次点击后才下载、验证签名、安装并重启应用。
