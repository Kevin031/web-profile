...
右侧脚本列表来自项目 `package.json` 的 `scripts`。点击脚本会自动生成对应包管理器的命令并立即启动：
...
- 点击项目行中的停止按钮，结束该项目及其子进程。
- 点击重启按钮，停止后使用已保存的命令重新启动。
- 点击顶部“全部停止”，结束所有由 Web Profile 启动的项目。
...
选中项目后，右侧“日志”区域显示启动命令、标准输出、错误输出和进程状态。默认每个项目最多保留 500 行日志；应用重启后不保证保留历史运行日志。
